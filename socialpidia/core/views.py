...
def index(request):
    ...
    #user feed(only shows posts by user and users that the user is following)
...
def like_post(request):
    ...
    #creates a record in the like post module if the user has not liked the post
...
def follow(request):
    if request.method == 'POST':
        ...
        #Unfollows the user if already following
        if FollowersCount.objects.filter(follower = follower, user = user).first():
            delete_follower = FollowersCount.objects.get(follower = follower, user = user)
            delete_follower.delete()
            return redirect('/profile/'+user)
        
        #follows them if not already following
        else:
